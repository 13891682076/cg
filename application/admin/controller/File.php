...
class File extends Base
{
    public function index($currdir=null){
        ...
        if($currdir){//如果目录存在
            if(file_exists($currdir)){//如果子目录存在
                //查看权限限制
                if(stripos($currdir,ROOT_PATH)===0 && stripos($currdir,ROOT_PATH."..")===false){//stripos查找字符串首次出现的位置
                    chdir($currdir);//切换子目录
                }
            }
        }
        ...
        while ($filename=readdir($dir)){
            if($filename!="." && $filename!=".."){
                if(is_dir($filename)){//如果是目录，
                    $arr['icon']="#icon-wenjianjia1";//添加目录图标
                    $arr['flag']=1;//添加目录标识
                    $num['dir']++;//目录遍历时统计
                }else{
        ...
        array_multisort(array_column($data,'flag'),SORT_DESC,$data);//多维数组排序，取出数组中的一列
        $curr=input('page');
    ...
    public function edit(){
        ...
        $arr=['.PHP','.CSS','.JS','.XML','.HTML','.HTACCESS','.TXT','.XLS','.DOC'];
